...
import ru.toughdev.ates.account.dto.UserTotalPayment;
...
public interface PaymentRepository extends JpaRepository<Payment, String> {
    ...
    @Query(value = "SELECT * FROM PAYMENTS p " +
            "WHERE p.user_id = :userId AND CAST(p.date_time AS date) = CURRENT_DATE() " +
            "ORDER BY p.date_time ASC",
            nativeQuery = true)
    List<Payment> findCurrentPaymentsByUserId(Long userId);
